...
def predict_disease_from_symptom(symptom_list):
    symptoms = {'itching': 0, 'skin_rash': 0, 'nodal_skin_eruptions': 0, 'continuous_sneezing': 0,
                ...
                'mild_fever': 0, 'yellow_urine': 0, 'yellowing_of_eyes': 0, 'acute_liver_failure': 0,
                'fluid_overload': 0, 'swelling_of_stomach': 0, 'swelled_lymph_nodes': 0, 'malaise': 0, 'blurred_and_distorted_vision': 0,
                ...
                'pain_in_anal_region': 0, 'bloody_stool': 0, 'irritation_in_anus': 0, 'neck_pain': 0, 'dizziness': 0,
                'cramps': 0, 'bruising': 0, 'obesity': 0, 'swollen_legs': 0, 'swollen_blood_vessels': 0, 'puffy_face_and_eyes': 0,
                'enlarged_thyroid': 0, 'brittle_nails': 0, 'swollen_extremeties': 0, 'excessive_hunger': 0,
                'extra_marital_contacts': 0, 'drying_and_tingling_lips': 0, 'slurred_speech': 0, 'knee_pain': 0,
                'hip_joint_pain': 0, 'muscle_weakness': 0, 'stiff_neck': 0, 'swelling_joints': 0, 'movement_stiffness': 0,
                'spinning_movements': 0, 'loss_of_balance': 0, 'unsteadiness': 0, 'weakness_of_one_body_side': 0,
                'loss_of_smell': 0, 'bladder_discomfort': 0, 'foul_smell_of urine': 0, 'continuous_feel_of_urine': 0,
                'passage_of_gases': 0, 'internal_itching': 0, 'toxic_look_(typhos)': 0, 'depression': 0, 'irritability': 0,
                'muscle_pain': 0, 'altered_sensorium': 0, 'red_spots_over_body': 0, 'belly_pain': 0, 'abnormal_menstruation': 0,
                'dischromic _patches': 0, 'watering_from_eyes': 0, 'increased_appetite': 0, 'polyuria': 0, 'family_history': 0,
                'mucoid_sputum': 0, 'rusty_sputum': 0, 'lack_of_concentration': 0, 'visual_disturbances': 0,
                'receiving_blood_transfusion': 0, 'receiving_unsterile_injections': 0, 'coma': 0, 'stomach_bleeding': 0,
                'distention_of_abdomen': 0, 'history_of_alcohol_consumption': 0, 'fluid_overload': 0, 'blood_in_sputum': 0,
                'prominent_veins_on_calf': 0, 'palpitations': 0, 'painful_walking': 0, 'pus_filled_pimples': 0,
    ...
    for i in range(len(symptom_list)):
        if symptom_list[i] in symptoms:
            symptoms[symptom_list[i]] = 1

    symptoms = list(symptoms.values())

    model = load("model.joblib")
    preds = model.predict([symptoms])

    df = pd.read_excel('DiseaseSymptom.xlsx')
    ...
    for col in df.columns[1:]:
        temp_dict[col] = df[df[col] == 1].index.tolist()

    preds = list(preds)
    if preds[0] in temp_dict:
        pred_label = preds[0]
        return pred_label, ' '.join(temp_dict[pred_label])

    else:
        return "No Disease Found", 'The symptoms you provided did not match any known diseases.'


def get_symtoms(dis):
    if dis.lower() == "chickenpox":
        return True, ["red skin rash", "itching", "spots"]

    elif dis.lower() == "dengue":
        return True, ["high fever", "rashes", "headache", "muscle pain"]

    elif dis.lower() == "malaria":
        return True, ["high fever", "sweating", "headache", "vomiting"]

    elif dis.lower() == "covid-19":
        return True, ["fever", "dry cough", "tiredness", "aches and pains"]
